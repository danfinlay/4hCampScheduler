...
function readyFunction(){
	...
	$('body').append('<canvas id="canvas" height = "'+screenHeight+'" width = "'+screenWidth+'"></canvas>');
	console.log("Canvas added to body with a width & height of "+screenWidth+"  "+screenHeight);
	...
	function generateSchedules(){
		...
		singleChorePermutations.forEach(function(workshopPermutation){
			...
			var workshopRectangle = canvas.display.rectangle({
				x: screenWidth*0.5,
				y: screenHeight*0.5,
			...
			$('h2').text("Workshops Generated");

			canvas.addChild(workshopRectangle);
			...
			for (var kpPermNumber = Math.floor(Math.random()*singleChorePermutations.length); kpPermNumber<singleChorePermutations.length;kpPermNumber=Math.floor(Math.random()*singleChorePermutations.length)){
				...
				$('h2').text("Generating KP Schedule");


				var angle = kpPermNumber * 350 / singleChorePermutations.length;
				var lineEndX= screenWidth*0.5 + screenWidth*0.3 * Math.cos(angle);
				var lineEndY= screenHeight*0.5 + screenWidth*0.3 * Math.cos(angle);
				var line = canvas.display.line({
					start: { x: screenWidth*0.5, y: screenHeight*0.5 },
				...
				while(!kpWorks){
					...
					if (noTribeHasKPAndFunInTheForestOnTheSameDay) {
						...
						$('h2').text("Enumerating John Duty Schedules...");
						line.animate({
							start: { x: screenWidth*0.5, y: screenHeight*0.5 },
							end: { x: lineEndX, y: lineEndY },
							stroke: "20px #0a0",
				...
				for(var jdPermNumber = 0; jdPermNumber<singleChorePermutations.length; jdPermNumber++){
					...
					if (jdPermNumber % 1000 === 0) {
						var arcAngle = jdPermNumber * 360 / singleChorePermutations.length;
						arc.animate({
							x: lineEndX,
							y: lineEndY,
							radius: screenHeight*0.1,
